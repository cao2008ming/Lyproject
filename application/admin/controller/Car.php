...
class Car extends AdminBase
{
    ...
    {
        if ($this->admin_id == 1){
            $car_list = $this->model->select();
            return $this->fetch('index_admin',['car_list'=>$car_list]);

        } else {
            $car_info = $this->model->where('user_id',$this->admin_id)->find();
            if ( empty($car_info) ) {
                return $this->fetch('index_user');
            } else {
                return $this->fetch('index',['car_info'=>$car_info]);
            }
        }
    }
    ...
    public function save()
    {
        if ($this->request->isPost()) {
            ...
            $validate_result = $this->validate($data, 'Car');
    ...
    {
        $info = $this->model->where('user_id',$id)->find();
    ...
    {
        $result = $this->model->where('id',$id)->setField('status',1);
    ...
    {
        $result = $this->model->where('id',$id)->setField('status',0);
    ...
    {
        $result = $this->model->where('id',$id)->setField('is_recommend',1);
    ...
    {
        $result = $this->model->where('id',$id)->setField('is_recommend',0);
